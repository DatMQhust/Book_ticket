...
export class UpdateTicket1762095371619 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."events_status_enum" AS ENUM('upcoming', 'ongoing', 'ended', 'cancelled')`,
    );
    await queryRunner.query(
      `ALTER TABLE "events" ADD "status" "public"."events_status_enum" NOT NULL DEFAULT 'upcoming'`,
    );
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."tickets_status_enum" AS ENUM('unchecked', 'checked_in')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "tickets" ALTER COLUMN "status" SET DEFAULT 'unchecked'`,
    );
    await queryRunner.query(`DROP TYPE "public"."tickets_status_enum_old"`);
    await queryRunner.query(
      `ALTER TABLE "ticket-types" ADD CONSTRAINT "FK_a9e4b6c604ca80e7c853b1e65bc" FOREIGN KEY ("eventId") REFERENCES "events"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "tickets" ADD CONSTRAINT "FK_bd5636236f799b19f132abf8d70" FOREIGN KEY ("order_id") REFERENCES "orders"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE "tickets" ADD CONSTRAINT "FK_a95369aeea12da7fde110e95e00" FOREIGN KEY ("ticket_type_id") REFERENCES "ticket-types"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "ticket-types" DROP CONSTRAINT "FK_a9e4b6c604ca80e7c853b1e65bc"`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."tickets_status_enum_old" AS ENUM('available', 'sold_out')`,
    );
    await queryRunner.query(
      `ALTER TABLE "tickets" ALTER COLUMN "status" DROP DEFAULT`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "tickets" ADD "sold" integer NOT NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE "tickets" ADD "description" text NOT NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE "tickets" ADD "rank" integer NOT NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE "tickets" ADD "quantity" integer NOT NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE "tickets" ADD "price" integer NOT NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE "tickets" ADD "name" character varying(200) NOT NULL`,
    );
